...
export default function ScoresPage() {
  ...
  const [form, setForm] = useState({ score: '', played_at: new Date().toISOString().split('T')[0], notes: '' })

  const fetchScores = async () => {
    if (!profile) return
    const { data } = await supabase.from('golf_scores').select('*')
      .eq('user_id', profile.id).order('played_at', { ascending: false })
    setScores((data as GolfScore[]) || [])
  }

useEffect(() => { fetchScores() }, [profile?.id])
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black" style={{ fontFamily: 'var(--font-display)' }}>My Scores</h1>
          <p className="text-white/40 text-sm mt-1">Your 5 most recent Stableford scores are your draw entries</p>
        </div>
      ...
      <div className="glass rounded-2xl p-4 mb-6 flex gap-3">
        ...
        <p className="text-white/50 text-sm leading-relaxed">
          Only your <strong className="text-white">5 most recent scores</strong> are stored. Adding a new score automatically removes the oldest one. Scores must be in Stableford format (1–45 points).
        </p>
      </div>
      ...
      {showForm && (
        <Card className="mb-6 border border-lime/20">
          <h3 className="text-lg font-bold mb-4" style={{ fontFamily: 'var(--font-display)' }}>Add Score</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            />
            <Button type="submit" loading={loading} fullWidth>Save Score</Button>
          </form>
        </Card>
      )}
      ...
      <Card>
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold" style={{ fontFamily: 'var(--font-display)' }}>Score History</h2>
        ...
        {scores.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-white/20 text-xs mt-1">Add your first Stableford score to enter the draw</p>
          </div>
        ) : (
          <div className="space-y-3">
            {scores.map((sc, i) => (
              <div key={sc.id}
                className={`flex items-center justify-between p-4 rounded-xl transition-all ${i === 0 ? 'bg-lime/8 border border-lime/15' : 'bg-white/3 border border-white/5'}`}>
                <div className="flex items-center gap-4">
                  <div className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center text-white/30 text-xs font-mono">
                    #{i + 1}
                  </div>
                  <div>
                    <p className="text-white font-semibold">{format(new Date(sc.played_at), 'EEEE, dd MMM yyyy')}</p>
                    {sc.notes && <p className="text-white/30 text-xs">{sc.notes}</p>}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-center">
                    <p className="text-2xl font-black text-lime" style={{ fontFamily: 'var(--font-mono)' }}>{sc.score}</p>
      ...
      {scores.length > 0 && (
        <Card className="mt-4 border border-lime/10">
          <p className="text-white/40 text-xs uppercase tracking-widest mb-4">Your Draw Numbers</p>
