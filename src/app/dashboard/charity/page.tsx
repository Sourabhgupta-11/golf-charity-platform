...
export default function CharityDashboardPage() {
  ...
  const [selected, setSelected] = useState<string | undefined>(profile?.charity_id)
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-black" style={{ fontFamily: 'var(--font-display)' }}>My Charity</h1>
        <p className="text-white/40 text-sm mt-1">Choose the cause you want to support with your subscription</p>
      </div>
      ...
      <Card className="mb-8 border border-coral/20">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="font-bold" style={{ fontFamily: 'var(--font-display)' }}>Your Contribution</h2>
        ...
        <div className="flex items-center justify-between glass rounded-xl p-4">
          <div>
            ...
            <p className="text-white/30 text-xs">Based on your {profile?.subscription_plan || 'monthly'} plan</p>
          </div>
      ...
      {/* Charity list */}
      <h2 className="text-lg font-bold mb-4" style={{ fontFamily: 'var(--font-display)' }}>Select a Charity</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {charities.map(charity => {
          const isSelected = selected === charity.id
          return (
            <button key={charity.id} onClick={() => setSelected(charity.id)}
              className={`rounded-2xl overflow-hidden text-left transition-all card-hover ${
                isSelected ? 'ring-2 ring-lime ring-offset-2 ring-offset-ink' : 'glass'
              }`}>
              {/* Image */}
